...
class GNSSOdom : public ParamServer {
 ...
 private:
  void GNSSCB(const sensor_msgs::msg::NavSatFix::ConstSharedPtr &msg) {
    ...
    if (distance > gpsHeadingInitDistance) {
      ...
      RCLCPP_WARN_THROTTLE(
          ...
          "Waiting for GPS heading initialization: moved %.2f / %.2f m",
          distance, gpsHeadingInitDistance);
      return;
    }
    ...
    tf2::Matrix3x3 yawRotation(tf2::Quaternion(
        yawQuat.x, yawQuat.y, yawQuat.z, yawQuat.w));
    tf2::Vector3 gpsOffsetBody(gpsOffset.x(), gpsOffset.y(), gpsOffset.z());
    tf2::Vector3 gpsOffsetOdom = yawRotation * gpsOffsetBody;
    Eigen::Vector3d calib_enu(enu(0) - gpsOffsetOdom.x(),
                              enu(1) - gpsOffsetOdom.y(),
                              enu(2) - gpsOffsetOdom.z());
